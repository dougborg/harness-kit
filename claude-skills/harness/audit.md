...
- A skill-local script referenced outside `${CLAUDE_SKILL_DIR}` or a shared helper referenced outside `${CLAUDE_SKILL_DIR}`
...
- A `${CLAUDE_SKILL_DIR}/<script>.sh` reference with no regular script in the skill directory, or a `${CLAUDE_SKILL_DIR}/<script>.sh` reference with no canonical helper — a dangling reference fails only at invocation time
